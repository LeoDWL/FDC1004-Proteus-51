...
void MyColorNum(u16 x,u16 y,u32 num,u16 size,u16 point_color,u16 back_color)
{
	u8 half_length;
	size=16;
	half_length=CountNumLength(num)<<2;
	LCD_Fill(x-half_length, y-8, x+half_length, y+8,back_color);
	POINT_COLOR=point_color;
	LCD_ShowNum(x-half_length,y-8,num,half_length>>2, size);
}
//显示数字,高位为0,还是显示
//x,y:起点坐标
//num:数值(0~999999999);	 
...
void MyColorStr(u16 x,u16 y,u8 *p,u16 size,u16 point_color,u16 back_color)
{
	u8 half_length;
	size=16;
	half_length=sizeof(p)<<3;
	LCD_Fill(x-half_length, y-8, x+half_length, y+8,back_color);
	POINT_COLOR=point_color;
	LCD_ShowString(x-half_length,y-8,half_length<<1,size, size, p);
}
